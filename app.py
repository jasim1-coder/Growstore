...
neuralModel = tf.keras.models.load_model('trainedModels/neuralNetwork.h5', compile=False)
neuralModel.compile(optimizer=tf.optimizers.Adam(learning_rate=0.001), loss='mean_squared_error')
...
# Define a function to calculate the fuzzy match score
...
def get_popular_products():
    try:
        ...
        df_rating = df.groupby("ProductID").agg({"UserID":"count","Rating":"mean"}).rename(columns={"UserID":"Number of Rating", "Rating":"Mean Rating"}).reset_index()
        ...
        df_filtered['score']=df_filtered.apply(weighted_rating, m=m, C=C, axis=1)
        df_highscore=df_filtered.sort_values(by='score', ascending=False).head(10)

        productIds = list(df_highscore.ProductID)
        
        products = list(productsModel.find({"_id": {"$in": productIds}}, {"title": 1}))
...
def related_products():
    try:
        ...
        # Get the product's title, description, and features
        productsModel = db.products
        productDetails = list(productsModel.find({"_id": product_id}, {'title': 1, 'description': 1}))
        product_data = productDetails[0]
        
        product_text = product_data['title'] + ' ' + product_data['description'] + ' '

        # Transform the product data using the SVD model
        product_transformed = svdModel.transform([product_text])

        # Calculate the cosine similarity between the product and all other products in the dataset
        similarity_matrix = cosine_similarity(product_transformed, svdModel.transform(products_df['title'] + ' ' + products_df['description']))

        # Get the indices of the top 5 most similar products
        top_indices = similarity_matrix.argsort()[0][-5:][::-1]

        # Get the IDs and titles of the top 5 most similar products
        top_ids = products_df.loc[top_indices, '_id'].tolist()
        
        products = list(productsModel.find({"_id": {"$in": top_ids}}, {"title": 1}))
...
def recommend():
    try:
        ...
        prediction_actual_id = ratings_df.loc[ratings_df["product_id"].isin(pred_ids)].reset_index(drop=True)
        prediction_actual_id_list = list(prediction_actual_id.ProductID.unique())
...
def searchProduct():
    ...
    new_df['match_score'] = new_df['title'].apply(lambda x: get_match_score(x, query))
